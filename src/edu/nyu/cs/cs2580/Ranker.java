...
class Ranker {
   ...
   * @author Amey
	 * @return
	 * returns the fquid - no. of occurences of query term i in document d
	 */
	...
	/**
	 * @author Amey
	 * returns the |D| - total no. of words in document D
	 * @return
	 */
	 ...
	 * @return
	 */
	public Vector<ScoredDocument> sortScoredDocumentAsPerScore(Vector<ScoredDocument> sds){
		if (sds.size() > 0){
			Collections.sort(sds, new Comparator<ScoredDocument>() {
	            @Override
	            public int compare(final ScoredDocument obj1, final ScoredDocument obj2) {
  ...
  /* Added by Ravi */
  /**
   * This function calculates the number of times a given term occurs in a document 
   * @param did
   * @param term
   * @return
   */
