...
public class LinearRanker {
	 ...
	 * @return
	 */
	public Vector<ScoredDocument> runquery(String query, float wCosine,
			float wLMP, float wPhrase, float wNumViews) {

		Vector<ScoredDocument> retrieval_results = new Vector<ScoredDocument>();
		double noOfDocs = _ranker.numDocs();
		for (int i = 0; i < noOfDocs; ++i) {
			retrieval_results.add(runquery(query, i, wCosine, wLMP, wPhrase,
					wNumViews));
	...
	public ScoredDocument runquery(String query, int did, float wCosine,
			float wLMP, float wPhrase, float wNumViews) {
		// Build query vector
		...
		double score = getLinearScore(query, did, wCosine, wLMP, wPhrase,
				wNumViews);
		return new ScoredDocument(did, d.get_title_string(), score);
	}

	public double getLinearScore(String query, int did, float wCosine,
			float wLMP, float wPhrase, float wNumViews) {
		double cosineScore = new CosineRanker(_ranker).getCosineScore(query,
				did);
		double phraseScore = new PhraseRanker(_ranker).getPhraseScore(query,
				did, 2);
